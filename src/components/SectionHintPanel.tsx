...
export function SectionHintPanel({
  ...
  return (
    <div className="section-hint-panel shrink-0 px-3 py-2 sm:px-5">
      {description ? (
        <p className="workspace-float-block flex items-start gap-2 px-2.5 py-2 text-[12px] leading-snug text-muted sm:text-xs">
      ...
      {exampleHint ? (
        <div className={description ? 'mt-2' : ''}>
          <button
            ...
            className="workspace-float-block flex w-full items-center justify-between gap-2 px-2.5 py-1.5 text-left text-[12px] font-medium text-ink transition-colors hover:border-border-strong sm:text-xs"
          ...
          {exampleExpanded ? (
            <div className="workspace-float-block mt-2 px-2.5 py-2">
              <p className="whitespace-pre-wrap text-[12px] leading-relaxed text-ink/85 sm:text-xs">
                {exampleHint}
              </p>
              {onApplyExample ? (
                <button
                  ...
                  className="mt-2 rounded-sm border-2 border-border/70 px-2.5 py-1 text-[12px] font-medium text-ink transition-colors hover:bg-surface-hover disabled:cursor-not-allowed disabled:opacity-50 sm:text-xs"
                >
